...
Result_by_Score_n_LabelFile = paste0("../Results/",Method,"_byTrial_",DrugSimNet,"_Score_n_Label.csv")
...
Methods = c("H1","H2","H3","MH12","MH13","MH23","MH123")
DrugSimNets = c("CHEM")#PREDICT/CHEM
...
#Store
SummaryFile = paste0("../Results/Summary_",paste0(Methods,collapse = "-"),"_",paste0(DrugSimNets,collapse = "-"),".rdata")
...
for(DrugSimNet in keys(h.All)){
  ...
  l.plot[[pi]] = p
  saveRDS(p,paste0(FigureFile,"_",DrugSimNet,".rdata"))#readRDS
}
library('cowplot')
l.plot[[1]]
# plot_grid(l.plot[[1]], l.plot[[2]], labels=c("A", "B"), ncol = 2, nrow = 1)

ggsave(paste0(FigureFile,"_",paste0(DrugSimNets,collapse = "-"),".pdf"), width = 7, height = 7)

#### read stored plot and plot then save to pdf file
library(ggplot2)
library('cowplot')
p1 = readRDS("~/Manuscripts/99HDR2/Figures/Figure_H1-H2-H3-MH12-MH13-MH23-MH123_PREDICT.rdata")

p2 = readRDS("~/Manuscripts/99HDR2/Figures/Figure_H1-H2-H3-MH12-MH13-MH23-MH123_CHEM.rdata")

plot_grid(p1, p2, labels=c("(a)", "(b)"), ncol = 2, nrow = 1)
ggsave("~/Manuscripts/99HDR2/Figures/Figure_H1-H2-H3-MH12-MH13-MH23-MH123_PREDICT_CHEM.pdf", width = 14, height = 7)
