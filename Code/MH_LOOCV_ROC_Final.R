...
for(Method in Methods){
  ...
  do_something <- function(disease_MultiplexObject,drug_MultiplexObject,
    ...
    EDRelation_disease <- DiDrRelation[which(DiDrRelation$diseaseid %in% disease_MultiplexObject$Pool_of_Nodes),]
    
    #Create multiplex-heterosgenous nw
    disease_drug_Net <- create.multiplexHet(disease_MultiplexObject, drug_MultiplexObject, 
                                                EDRelation_disease)
    
    diseaseHetTranMatrix <- compute.transition.matrix(disease_drug_Net, delta1=delta, delta2=delta)
    
    #compute 
    #tau <- c(1,1)
    RWRH_disease_drug_Results <- Random.Walk.Restart.MultiplexHet(diseaseHetTranMatrix,
                                                                      disease_drug_Net,SeedDisease,
                                                                      seeddrug, r = gamma)
                                                                      
    
    #create labels for disease results
    tf = RWRH_disease_drug_Results$RWRMH_Multiplex1
    tf$labels <- ifelse(tf$NodeNames==prd_disease, 1, 0)
    
    # calculating AUC for each disease of a drug
