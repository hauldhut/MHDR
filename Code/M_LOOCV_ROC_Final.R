...
delta = 0.5
gamma = 0.5#gamma
for(delta in c(0.3, 0.7, 0.9)){#0.1, 
  ...
  DiSimNet3 <- read.delim("../Data/DiseaseSimNet_HPO.sif",header = FALSE)
  ...
  for (i in 1:length(DiDr.frame$diseaseid)) {
    ...
    if (length(disease_relation$diseaseid)>=2) {
      ...
      RWR_disease_Results <- Random.Walk.Restart.Multiplex(AdjMatrixNorm_disease,
                                                            disease_MultiplexObject,
                                                            Seeddisease,tau = tau, r = gamma)
  
      tf = RWR_disease_Results$RWRM_Results
      #create labels for disease results
      tf$labels <- ifelse(tf$NodeNames==prd_disease, 1, 0)
      
      # calculating AUC for each disease of a disease
  ...
  Result_byTrialFile = paste0("../Results/",Method,"_byTrial_",BipartiteNet,"_delta",delta,"_ROC.csv")
  ...
  Result_by_Score_n_LabelFile = paste0("../Results/",Method,"_byTrial_",BipartiteNet,"_delta",delta,"_Score_n_Label.csv")
