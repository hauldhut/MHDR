...
class WeightedGATConv(GATConv):
    def __init__(self, in_channels, out_channels, heads=1, concat=True, dropout=0.0, add_self_loops=True):
        super(WeightedGATConv, self).__init__(in_channels, out_channels, heads=heads, concat=concat, dropout=dropout, add_self_loops=add_self_loops)
    
    def forward(self, x, edge_index, edge_weight=None):
        # GATConv uses edge_attr for additional edge features, but we'll adapt edge_weight
        # Reshape edge_weight to match the expected edge_attr shape (num_edges, 1)
...
class GAT(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, heads=4, dropout=0.5):
        ...
        self.conv1 = WeightedGATConv(input_dim, hidden_dim, heads=heads, concat=True, dropout=dropout)
        # After concat, the output dimension of conv1 is hidden_dim * heads
        self.conv2 = WeightedGATConv(hidden_dim * heads, output_dim, heads=1, concat=False, dropout=dropout)
        self.dropout = dropout
    
    def forward(self, data):
        x, edge_index, edge_weight = data.x, data.edge_index, data.edge_weight
        x = self.conv1(x, edge_index, edge_weight=edge_weight)
        x = F.elu(x)  # ELU activation as recommended in the GAT paper
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = self.conv2(x, edge_index, edge_weight=edge_weight)
        return x

# Step 5: Train GAT model with unsupervised loss (graph reconstruction)
def train_gat(data, embedding_size=128, hidden_dim=64, epochs=20, lr=0.01, node_type="nodes"):
...
# Main function
def main(drug_sim_file, disease_sim_file, embedding_size=64, epochs=200, output_file="embeddings.csv"):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Learn GAT embeddings from drug and disease similarity networks")
